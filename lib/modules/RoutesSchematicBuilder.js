const fs = require('fs');
const { X_PREFIX, Colors, V_PREFIX } = require('../constants');
        ...
        for(const [categoryName, categoryValue] of categories) {
            ...
            for(const [subCategoryName, settings] of subCategories) {
                ...
                if(fs.existsSync(routeFilePath)) {
                    console.log(`${X_PREFIX}Skipping, route already exists in ${Colors.Red}${routeFilePath}${Colors.None}`);
                    continue;
                }

                console.log(`${V_PREFIX}Created route in ${Colors.Green}${routeFilePath}${Colors.None}`)
