...
const { Colors, X_PREFIX, V_PREFIX } = require('../constants');
...
class RouteInitializer {
    ...
    init(apiPrefix, directory) {
        if(!this.app) return console.log(`${X_PREFIX}Cannot use ${Colors.Red}RouteInitializer.init()${Colors.None} method when app param was not provided`);
        ...
        for(const route of routes) {
            ...
            const analyzeSchematic = (schematic, req, res) => {
                ...
                const analyzePropertieses = (param, value, obj) => {
                    if("type" in obj) {
                        if(obj.type === "string") return { error: true, message: `Param "${param}" has to be string` }
                        if(obj.type === "number" && isNaN(value)) return { error: true, message: `Param "${param}" has to be number` }
                        if(obj.type === "json" && !isJson(value)) return { error: true, message: `Param "${param}" has to be json` }
                    }
